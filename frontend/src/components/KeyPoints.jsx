...
export default function KeyPoints({ keyPoints, loading, onGenerate }) {
  const [copied, setCopied] = useState(false);

  const handleCopy = () => {
    if (!keyPoints) return;
    navigator.clipboard.writeText(keyPoints);
    setCopied(true);
    ...
    <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl relative backdrop-blur-sm">
      <div className="flex items-center justify-between gap-4 pb-6 border-b border-slate-800 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-100">Key Points & Contributions</h2>
            <p className="text-xs text-slate-400">Core findings, methodology highlights, and novel results</p>
          </div>
        </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>{keyPoints ? 'Regenerate' : 'Extract Key Points'}</span>
              </>
            )}
          </button>
        </div>
      ...
      <div className="pt-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
            ...
            <p className="text-sm text-slate-400">Distilling key points & methodologies...</p>
          </div>
        ) : keyPoints ? (
          ...
          <div className="text-center py-12 text-slate-500 space-y-2">
            ...
            <p className="text-xs">Click the button above to extract key takeaways from the paper.</p>
          </div>
        )}
      </div>
    </div>
